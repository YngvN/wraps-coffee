...
- **Mock data** for the menu, cafe events and customer reviews lives in `src/data/menu.json`, `src/data/events.json` and `src/data/reviews.json`, standing in for a future API. `src/utils/events.ts` computes event occurrences — the next occurrence of each event for previews, or every occurrence within a date range for the calendar — including recurring weekly events, postponed dates and cancelled occurrences; `src/utils/calendar.ts` provides the underlying month/week grid date math.
