...
- **Home page** (`/`) with a full-bleed hero introduction featuring the next upcoming event (with image, sourced from `src/data/events.json`) — on wide screens this expands into a gallery showing the next 3 upcoming events, with a large picture that swaps to match whichever event is hovered — a full-screen "Good food that fills you up" section with a cross-fading carousel of customer reviews (`ReviewCarousel`, sourced from `src/data/reviews.json`) alongside a short summary and an "Order now" button, and a "Find us" section with an OpenStreetMap embed of the cafe's location. The hero sits behind a transparent header; scrolling down animates the hero out of view and reveals the "Wraps & Coffee" name in the header, while the nav links stay fixed in a sticky pill. On first load, the hero items "rain in" from below in a staggered sequence, and each section below it fades and slides into place (headings from below, content from the left/right according to its position) the first time it scrolls into view (`src/hooks/useReveal.ts`, `src/utils/motion.ts`), respecting `prefers-reduced-motion`.
...
- **Reusable UI components** (`Button`, `Input`, `Card`, `Badge`, `Alert`, `Modal`, `EventDetailsModal`, `ReviewCarousel`, `Spinner`, `Checkbox`, `LocationMap`, and more) exported from `src/components`, with a live preview at the `/components` route.
- **Mock data** for the menu, cafe events and customer reviews lives in `src/data/menu.json`, `src/data/events.json` and `src/data/reviews.json`, standing in for a future API. `src/utils/events.ts` computes the next occurrence of each event, including recurring weekly events, postponed dates and cancelled occurrences.
