...
- **AI assistant** (the sparkle icon in the admin top navbar, `src/features/admin/assistant`, `server/assistant`) — a Claude-powered chatbox that creates, updates, or deletes data across most of the dashboard from a plain-language request, in whichever language the admin's own dashboard is set to: products, catalogues, categories, events, non-admin user accounts, message boards and posts, appearance themes and theme colors, store branding, contact info, and integration on/off toggles. It never writes anything itself — every request only ever stages a draft, shown in the exact same form used elsewhere in the dashboard, and the admin has to explicitly confirm (typing the item's own name out, for a delete) before anything is actually saved or removed. When a request leaves a required field genuinely ambiguous (e.g. no category named for a new product), it asks a short multiple-choice clarifying question instead of guessing at one. Configured with a Claude API key from the Integrations page, same as the Wolt/Foodora integrations.
...
- **[@anthropic-ai/sdk](https://github.com/anthropics/anthropic-sdk-typescript)** — Claude API client powering the AI assistant chatbox (`server/assistant/`): intent routing, item selection, and field extraction from text or an attached image, each behind a forced tool call so the model can only ever pick from a small, real set of choices.
