...
const TOGGLEABLE_SIDEBAR_ITEMS: readonly string[] = ['messages', 'products', 'events', 'orders', 'screens', 'media', 'messageboard'] satisfies ToggleableSidebarItem[]
...
/** Drops (or remaps, see `LEGACY_ITEM_ALIASES`) any persisted entry that isn't a current `ToggleableSidebarItem` — covers legacy values from before Store settings, Integrations, and Display Manager became submenus (first Store's `'contact'`, briefly remapped to `'store'`; then `'displaymanager'` itself once it moved under Screens; then `'extensions'` — the old name for Integrations — once it moved under Settings), none of which are real sidebar items anymore. A pure read-time filter, never persisted back, so it's cheap to just re-apply on every read. */
