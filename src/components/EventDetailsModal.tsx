...
export function EventDetailsModal({ event, onClose }: EventDetailsModalProps) {
  ...
  return (
    <Modal open={event !== null} onClose={onClose} title={event?.title[language]}>
      {/* Keyed by eventID so switching directly from one event to another
          remounts this (resetting its registration step/form fields to
          fresh initial state) rather than needing an effect to reset them. */}
      {event && <EventDetailsContent key={event.eventID} event={event} />}
    </Modal>
  )
}
...
/** The stateful body of `EventDetailsModal` for a single event — split out so remounting it (via a `key`) is what resets its sign-up form between events. */
function EventDetailsContent({ event }: EventDetailsContentProps) {
  const { t, language } = useLanguage()
  const occursAt = getNextOccurrence(event)
