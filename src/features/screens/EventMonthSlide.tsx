...
import { formatEventDate, formatEventPrice, getEventsInMonth, isEventFree } from '../../utils/events'
      ...
      {monthEvents.length === 0 ? (
        ...
        <ul className="event-month-slide__list">
          {monthEvents.map(({ event, occursAt, status }, index) => (
            <li key={`${event.eventID}-${index}`} className={`event-month-slide__item${status === 'cancelled' ? ' event-month-slide__item--cancelled' : ''}`}>
