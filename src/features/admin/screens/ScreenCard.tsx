...
/**
 * One screen's own card in the admin Screens list: a static screenshot on
 * top (`screen.previewImages`, regenerated by `screenPreviewCapture.ts`
 * every time the screen is saved/published — see `ScreenForm.tsx`'s and
 * `ScreenDisplay.tsx`'s own save/publish handlers), letterboxed into a
 ...
 * `previewAspectRatio`. Falls back to a real, read-only live `SplitLayout`
 * render (the exact same live renderer the screen editor's "Layout" tab
 * uses) for a screen with no screenshot yet, or fewer than `stageCount` of
 * them — bounded to just that one card, and self-healing the next time
 * that screen is saved — rather than mounting a live render for every card
 * unconditionally, which independently ran its own clocks/polling/video
 * playback per card and was the whole reason this became a screenshot in
 * the first place. A screen with more than one step gets its own
...
export function ScreenCard({ screen, url, editorUrl, defaultPaneLanguage, slotCountLabel, copied, onCopy, onOpen, onOpenEditor, onEdit, onDuplicate, onDelete }: ScreenCardProps) {
  ...
  const stagePreviewImage = screen.previewImages?.[previewStage - 1]
  const previewImagesComplete = (screen.previewImages?.length ?? 0) >= stageCount
    ...
    <div className="screen-card">
      <div className="screen-card__preview" style={getScreenColorVars(screen.backgroundColor ?? DEFAULT_SCREEN_BACKGROUND_COLOR) as CSSProperties}>
        <ScaledScreenPreview aspectRatio={screen.previewAspectRatio} fit="contain">
          {stagePreviewImage && previewImagesComplete ? (
            <img className="screen-card__preview-image" src={getSmallUrl(stagePreviewImage)} alt="" />
          ) : (
            <SplitLayout
              screen={screen}
