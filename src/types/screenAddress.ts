 ...
 * link — `mdns` (the default) has the local server itself advertise a
 ...
 * `automatic` auto-detects this machine's own LAN IP instead, which can
 * change on a router/computer restart; `custom` uses an admin-typed
 * hostname/IP (e.g. one they've DHCP-reserved on their router).
...
export const DEFAULT_SCREEN_ADDRESS_SETTINGS: ScreenAddressSettings = { mode: 'mdns' }
