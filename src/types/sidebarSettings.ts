/** A sidebar item a store can opt out of showing entirely — every `AdminSidebarNav` item except `overview` (the dashboard home) and `settings` (where this very list is edited), both of which stay visible unconditionally. Store settings (company name/logo/favicon + nested Contact info), Integrations, and Display Manager no longer have their own sidebar entries — Store and Integrations moved under Settings, Display Manager moved under Screens, all as submenus — so none of them are part of this list. */
export type ToggleableSidebarItem = 'messages' | 'products' | 'events' | 'orders' | 'screens' | 'media' | 'messageboard'
