/** A sidebar item a store can opt out of showing entirely — every `AdminSidebarNav` item except `overview` (the dashboard home) and `settings` (where this very list is edited), both of which stay visible unconditionally. Store settings (company name/logo/favicon + nested Contact info) and Integrations have no sidebar entries of their own — both are submenus under Settings — so neither is part of this list. Displays *is* its own top-level item and so is toggleable here. */
export type ToggleableSidebarItem = 'messages' | 'products' | 'events' | 'orders' | 'screens' | 'displays' | 'media' | 'messageboard'
