...
): content is Extract<ScreenSlotContent, { kind: 'category' } | { kind: 'menu' } | { kind: 'events' } | { kind: 'transit' } | { kind: 'weather' }> {
  return content.kind === 'category' || content.kind === 'menu' || content.kind === 'events' || content.kind === 'transit' || content.kind === 'weather'
}
